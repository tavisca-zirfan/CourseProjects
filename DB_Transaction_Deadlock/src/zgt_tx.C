...
void *readtx(void *arg){
  ...
  // if tx is not null   
  if(tx!=NULL && tx->set_lock(node->tid, tx->sgno, node->obno, node->count, 'S')==0){
    if(tx->status!=TR_ABORT || tx->status!=TR_END)
      tx->perform_readWrite(node->tid,node->obno, 'S');   
  }
  finish_operation(node->tid);
  pthread_exit(NULL);       // thread exit
...
void *writetx(void *arg){ //do the operations for writing; similar to readTx
  ...
  if(tx!=NULL && tx->set_lock(node->tid, tx->sgno, node->obno, node->count, 'X')==0){
      if(tx->status!=TR_ABORT || tx->status!=TR_END)
        tx->perform_readWrite(node->tid,node->obno, 'X'); 
  }
  finish_operation(node->tid);
  pthread_exit(NULL);       // thread exit
...
void *do_commit_abort(long t, char status){ 
  ...
  if(tx!=NULL && tx->status!=TR_ABORT){ 
    ...
    tx->free_locks();     
    //if(tx->semno==tx->tid){
    for(int i =0;i<zgt_nwait(tx->tid);i++){  
     zgt_v(tx->tid);
    }
    //}
    tx->remove_tx();
...
int zgt_tx::set_lock(long tid1, long sgno1, long obno1, int count, char lockmode1){
  ...
  // if already there is a lock
  else if(this->status!=TR_ABORT && ep->tid!=tid1){ 
    zgt_tx* otherTx = get_tx(ep->tid); 
    this->status = TR_WAIT;       
    this->obno = obno1;
    this->lockmode = lockmode1;  
    this->setTx_semno(ep->tid,ep->tid);   
    wait_for* wfg = new wait_for();
    int noOfdeadlockRemoved = wfg->deadlock(); 
    zgt_v(0);    
    if(noOfdeadlockRemoved>0){ 
      return this->set_lock(tid1,  sgno1,  obno1,  count,  lockmode1);
    }
    this->print_tm();
    if(otherTx->status!=TR_ABORT && otherTx->status!=TR_END ) { 
      zgt_p(ep->tid);
      //this->status = this->status==TR_WAIT?TR_ACTIVE:this->status;
      return this->set_lock(tid1,  sgno1,  obno1,  count,  lockmode1);      
    }
    return this->set_lock(tid1,  sgno1,  obno1,  count,  lockmode1);
  }
...
void zgt_tx::print_wait(){
  ...
  printf("\n\t%d\t%d\t%d\t%d\t%d\t%d\t%d",this->sgno,this->Txtype,this->obno,this->tid,this->pid,this->semno,this->lockmode);
  printf("\n");
}
